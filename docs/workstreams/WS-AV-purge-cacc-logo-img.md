# WS-AV — Purge cacc-logo.svg `<img>` from the code entirely

- Branch: ws/purge-cacc-logo-img
- PR title: [ws/purge-cacc-logo-img] WS-AV-purge-cacc-logo-img: confirm cacc-logo.svg img is absent from the code
- Depends on: none
...
- Repo-wide search for `cacc-logo`, `California Cadet Corps logo`, and `<img` (excluding `.git`/`node_modules`/`package-lock.json`) returns **zero** matches.
- No `cacc-logo.svg` asset exists anywhere in the tree (`find . -name '*.svg'` → none).
- `src/server.js` is a static file server (it only maps `.svg` → `image/svg+xml` in its MIME table at line 20); it has no inline markup referencing the logo.

## Root cause

There is nothing to delete. Git history shows the `<img src="/cacc-logo.svg" …>` element was introduced earlier (WS-A `add-cacc-logo`) and was already removed by later workstreams (WS-AG, WS-AP), with subsequent confirm-only contracts (WS-AT, WS-AU). The desired end state — landing page with no `cacc-logo.svg` `<img>` — already holds on `main`. This WS exists to **verify and confirm** that state, not to re-apply a change.

## Scope

No source change is expected. The coder verifies, and only edits if a stray reference is found.

- `src/public/index.html` — confirm there is no `<img>` tag and no `/cacc-logo.svg` reference. If one is somehow present, remove that single line so the `<main>` block matches lines 21–26 as they read today; otherwise leave the file byte-for-byte unchanged.
- No other files require changes. Do **not** add, remove, or rename `cacc-logo.svg` (it does not exist).

## Acceptance / DoD

- A repo-wide search (`grep -rn 'cacc-logo' src/` and across the tree) returns no matches (already true).
...
Read the WS file at docs/workstreams/WS-AV-purge-cacc-logo-img.md in full before doing anything.

Work on branch `ws/purge-cacc-logo-img` in worktree `cacc-ws-purge-cacc-logo-img`.
